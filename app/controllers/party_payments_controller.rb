class PartyPaymentsController < ApplicationController
  ...
  def monthly_and_yearly_report
    ...
    @reports = BankBalanceOut.where(:created_at => Date.today.beginning_of_month..Date.today.end_of_month)
  ...
  def monthly_and_yearly_report_search
    ...
    if params[:month_id] !='' && params[:year_id] != ''
      ...
      @reports = BankBalanceOut.where(:created_at => @year_month.beginning_of_month..@year_month.end_of_month)
    elsif params[:year_id] != ''
      @year_month= DateTime.new( params[:year_id].to_i)
      @reports = BankBalanceOut.where(:created_at => @year_month.beginning_of_year..@year_month.end_of_year)
    else
      @reports = BankBalanceOut.all
    end
  ...
  def date_to_date_report_search
    ...
    @reports = BankBalanceOut.where(:created_at => @start_date.beginning_of_day..@end_date.end_of_day)
    if @bank_account
      @reports = @reports.where(:bank_account_id => @bank_account.id)
    end
  end
